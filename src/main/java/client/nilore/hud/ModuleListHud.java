...
import client.nilore.modules.impl.render.ModuleElement;
...
public class ModuleListHud extends HudElement {
    ...
    // ========== 后缀括号开关 ==========
    private BooleanSetting suffixBracketsEnabled;  // 控制是否显示 [] 括号

    // ========== 后缀颜色选择设置 ==========
    private ModeSetting suffixColorMode;  // 选择20种颜色之一
    ...
    public void registerSettings() {
        ...
        // ========== 20种后缀颜色选择 ==========
        this.suffixColorMode = new ModeSetting("Suffix Color", "LightPink",
                "Red", "Gold", "Yellow", "Green", "DarkGreen",
                "Aqua", "DarkAqua", "Blue", "DarkBlue", "Pink",
                "Purple", "Gray", "DarkGray", "White", "DarkRed",
                "Black", "LightRed", "LightGreen", "LightAqua", "LightPink");
    ...
    /**
     * 获取模块的后缀颜色 - 同步ModuleElement的颜色设置
     */
    private int getSuffixColor(Module module) {
        ...
        // 优先使用ModuleElement同步的颜色
        int syncedColor = ModuleSuffixColorManager.getSuffixColor(module);
        if (syncedColor != 0xFF888888) {
            return syncedColor;
        }
    ...
    private void drawModuleName(DrawContext drawContext, AnimatedRow row, float rowX, float rowY, float rowWidth, float rowHeight,
        ...
        // 判断是否有后缀
        boolean hasSuffix = this.showSuffix.getValue() && this.suffixColorEnabled.getValue() && text.contains(" ");
            ...
            if (FONT_GLOW_ENABLED) {
                ...
                if (radius > 0.0f && glowAlphaValue > 0) {
                    ...
                    for (int i = 0; i < quality; i++) {
                        ...
                        if (hasSuffix && suffixPart.length() > 0) {
                            GlHelper.drawText(namePart, textX + ox, textY + oy, font, Argb.withAlpha(color, animatedGlowAlpha / quality));
                            float suffixX = textX + GlHelper.getStringWidth(namePart, font) + 2.0f;
                            GlHelper.drawText(suffixPart, suffixX + ox, textY + oy, font, Argb.withAlpha(suffixColor, animatedGlowAlpha / quality));
                        } else {
