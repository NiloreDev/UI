...
public class NoXZMode
    ...
    private int getAttackCount(ClientboundSetEntityMotionPacket motionPacket) {
        if (!Velocity.INSTANCE.autoAttackCount.getValue() || motionPacket == null) {
            return Velocity.INSTANCE.attackAmount.getValue().intValue();
        }
    ...
    private void doAttackSequence(TickEvent tickEvent) {
        ...
        if (this.attacksRemaining <= 0) {
            this.clearTarget();
            if (Velocity.INSTANCE.instantAttack.getValue()) {
                if (Velocity.INSTANCE.debugLog.getValue()) {
                    ChatUtil.print("Attack (" + Velocity.INSTANCE.attackAmount.getValue().intValue() + ")");
                }
            }
        }
    }
    ...
    private boolean doAttack(Entity entity) {
        ...
        if (Velocity.INSTANCE.sprintStateCheck.getValue() && !mc.player.isSprinting()) {
            if (Velocity.INSTANCE.debugLog.getValue()) {
