...
public class InventoryManager extends Module {
    ...
    // 格式与 ChestStealer 保持一致: [Delay:xxx]
    // ================================================================
    @Override
    public String getSuffix() {
        long delay = this.actionDelaySetting.getValue().longValue();
        return "[Delay:" + delay + "]";
    }
    ...
    private boolean isUsingItem() {
        ...
        return mc.player.isUsingItem()
                || mc.player.getUseItem().getItem() != Items.AIR;
    }

    private boolean performInventoryAction() {
        // 使用方块、食物、弓、弩、钓竿等物品时禁止 InventoryManager 进行整理/丢弃
        // 避免 ClickType.THROW 导致正在使用的物品异常丢失
        if (isUsingItem()) {
            return false;
        }
